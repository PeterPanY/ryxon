// @ts-nocheck
import { Comment, computed, getCurrentInstance, h, ref, watchEffect } from 'vue'
...
function useRender<T>(
  ...
  const checkSubColumn = (children: TableColumn<T> | TableColumn<T>[]) => {
    function check(item: TableColumn<T>) {
      if (item?.type?.name === 'RTableColumn') {
  ...
  const setColumnRenders = (column: TableColumnCtx<T>) => {
    ...
    if (column.type === 'expand') {
      ...
      owner.value.renderExpanded = (data) =>
        slots.default ? slots.default(data) : slots.default
    } else {
      ...
      column.renderCell = (data) => {
        ...
        if (column.showOverflowTooltip) {
          props.class = `${props.class} r-tooltip`
          props.style = {
            width: `${
