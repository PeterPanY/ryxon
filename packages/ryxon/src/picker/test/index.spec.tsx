...
test('should emit confirm event after clicking the confirm button', async () => {
  ...
  await wrapper.find('.r-picker__confirm').trigger('click')
...
test('should emit cancel event after clicking the cancel button', () => {
  ...
  wrapper.find('.r-picker__cancel').trigger('click')
...
test('should emit change event after draging the column', () => {
  ...
  triggerDrag(wrapper.find('.r-picker-column'), 0, -100)
  wrapper.find('.r-picker-column ul').trigger('transitionend')
...
test('should emit change event when after clicking a option', async () => {
  ...
  await wrapper.findAll('.r-picker-column__item')[1].trigger('click')
  expect(wrapper.emitted('change')).toEqual([
    [
      {
        columnIndex: 0,
...
test('should emit click-option event after clicking an option', async () => {
  ...
  await wrapper.find('.r-picker-column__item').trigger('click')
...
test('should allow to set rem option height', async () => {
  ...
  await later()
  expect(wrapper.find('.r-picker-column__item').style.height).toEqual('160px')
...
test('should not allow to change option when using readonly prop', async () => {
  ...
  triggerDrag(wrapper.find('.r-picker-column'), 0, -100)
  await wrapper.find('.r-picker-column ul').trigger('transitionend')
  await wrapper.findAll('.r-picker-column__item')[3].trigger('click')
...
test('should not render mask and frame when options is empty', async () => {
  ...
  expect(wrapper.find('.r-picker__mask').exists()).toBeFalsy()
  expect(wrapper.find('.r-picker__frame').exists()).toBeFalsy()

  await wrapper.setProps({ columns: [{ values: ['foo'] }] })
  expect(wrapper.find('.r-picker__mask').exists()).toBeTruthy()
  expect(wrapper.find('.r-picker__frame').exists()).toBeTruthy()
})

test('columns-input-names responsiveness', async () => {
  const columnsOne = [
    { type: 1, name: 'Ios' },
    { type: 2, name: 'Android' }
  ]
  ...
  expect(wrapper.findAll('.r-ellipsis')[0].text()).toEqual('Ios')
  await wrapper.setProps({
    columns: columnsTwo,
    columnsInputNames: { text: 'serverName' }
  })
  expect(wrapper.findAll('.r-ellipsis')[0].text()).toEqual('server1')
})
