...
export const getTimeLists = (
  ...
  const getHoursList = (role: string, compare?: Dayjs) =>
    makeList(24, disabledHours && (() => disabledHours?.(role, compare)))
...
export const buildAvailableTimeSlotGetter = (
  ...
  const getAvailableHours: GetDisabledHoursState = (role, compare?) =>
    makeAvailableArr(getHoursList(role, compare))

  const getAvailableMinutes: GetDisabledMinutesState = (hour, role, compare?) =>
    makeAvailableArr(getMinutesList(hour, role, compare))
