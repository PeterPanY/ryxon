...
export default {
  setup() {
    const active = ref(0)
    const onChange = (index) => showMessage(`标签 ${index}`)
    return {
      icon,
      onChange
    }
...
| to | 点击后跳转的目标路由对象，等同于 Vue Router [to 属性](https://router.vuejs.org/zh/api/interfaces/RouterLinkProps.html#Properties-to) | _string \| object_ | - |
