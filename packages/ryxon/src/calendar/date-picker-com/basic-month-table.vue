<template>
  <table
    role="grid"
    :aria-label="t('rDatepicker.monthTablePrompt')"
    :class="bem()"
    ...
    <tbody ref="tbodyRef">
      <tr v-for="(row, key) in rows" :key="key">
        <td
          ...
          :aria-label="t(`rDatepicker.month${+cell.text + 1}`)"
        ...
        >
          <div>
            <span class="cell">
              {{ t('rDatepicker.months.' + months[cell.text]) }}
            </span>
          </div>
        </td>
      </tr>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    const [, bem, t] = createNamespace('month-table')
