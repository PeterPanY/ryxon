...
test('should limit max range when using max-range prop and type is range', async () => {
  ...
  const days = wrapper.findAll('.r-calendar__day');
...
test('show-title prop', async () => {
  ...
  expect(wrapper.find('.r-calendar__header-title').exists()).toBeTruthy();
  await wrapper.setProps({ showTitle: false });
  expect(wrapper.find('.r-calendar__header-title').exists()).toBeFalsy();
});
...
test('show-subtitle prop', async () => {
  ...
  expect(wrapper.find('.r-calendar__header-subtitle').exists()).toBeTruthy();
  await wrapper.setProps({ showSubtitle: false });
  expect(wrapper.find('.r-calendar__header-subtitle').exists()).toBeFalsy();
});
...
test('hide close icon when there is no title', async () => {
  ...
  expect(wrapper.find('.r-popup__close-icon').exists()).toBeTruthy();

  await wrapper.setProps({
    showTitle: false,
    showSubtitle: false,
  });
  expect(wrapper.find('.r-popup__close-icon').exists()).toBeFalsy();
});
...
test('min-date after current time', () => {
  ...
  wrapper.find('.r-calendar__confirm').trigger('click');
...
test('first day of week', async () => {
  ...
  expect(wrapper.findAll('.r-calendar__weekday')[0].text()).toEqual('Tue');

  const day = wrapper.find(
    '.r-calendar__month:first-of-type .r-calendar__day'
  );
...
test('should disabled prompt when using show-range-prompt prop', async () => {
  ...
  await later();
  expect(document.querySelector('.r-toast')).toBeFalsy();
});
