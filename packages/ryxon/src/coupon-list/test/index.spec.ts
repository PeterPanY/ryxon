...
test('should have two "r-coupon-list__empty" classes when render coupon list is empty', async () => {
  const wrapper = mount(CouponList, {
    props: {
      coupons: [],
      disabledCoupons: []
...
test('should emit "exchange" event when exchange button is clicked', async () => {
  const wrapper = mount(CouponList)
  const exchange = wrapper.find('.r-coupon-list__exchange')
  const input = wrapper.find('.r-input__control')
