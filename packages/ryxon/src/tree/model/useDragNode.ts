...
export function useDragNodeHandler({ props, ctx, el$, dropIndicator$, store }) {
  ...
  const treeNodeDragOver = ({ event, treeNode }: DragOptions) => {
    ...
    if (oldDropNode && oldDropNode !== dropNode) {
      removeClass(oldDropNode.$el, isBem('drop-inner'))
    }
    const { draggingNode } = dragState.value
    ...
    const iconPosition = dropNode.$el
      .querySelector(`.${bem('node-expand-icon')}`)
    ...
    if (dropType === 'inner') {
      addClass(dropNode.$el, isBem('drop-inner'))
    } else {
      removeClass(dropNode.$el, isBem('drop-inner'))
    }
