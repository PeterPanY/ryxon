...
function useRender<T>(props: Partial<TableBodyProps<T>>) {
  ...
  const firstDefaultColumnIndex = computed(() =>
    props.store.states.columns.value.findIndex(({ type }) => type === 'default')
  )
  const getKeyOfRow = (row: T, index: number) => {
    const { rowKey } = parent.props as Partial<TableProps<T>>
    if (rowKey) {
  ...
  const cellChildren = (cellIndex, column, data) => column.renderCell(data)
  ...
  const rowRender = (
    ...
    let display = true
    if (treeRowData) {
      rowClasses.push(bem('row', `level-${treeRowData.level}`))
              ...
              [
                h(
                  'td',
                  {
                    ...
                    class: `${bem('cell')} ${bem('expanded-cell')}`
                  },
