...
function useWatcher<T>() {
  ...
  // 检查 rowKey 是否存在
  const assertRowKey = () => {
    if (!rowKey.value) throw new Error('[RTable] prop row-key is required')
  }

  // 选择
  const isSelected = (row) => selection.value.includes(row)
  ...
  const getSelectionRows = () => (selection.value || []).slice()
  ...
  const updateAllSelected = () => {
    ...
    let selectedMap: Record<string, { row: T; index: number }>
    if (rowKey.value) {
