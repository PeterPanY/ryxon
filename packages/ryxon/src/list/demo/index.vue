...
import RPullRefresh from '../../pull-refresh';
import { ref } from 'vue';
  ...
  <r-tabs>
    <r-tab :title="t('basicUsage')">
      <r-list
        ...
        @load="onLoad(0)"
      >
        <r-cell v-for="item in list[0].items" :key="item" :title="item" />
      </r-list>
    </r-tab>

    <r-tab :title="t('errorInfo')">
      <r-list
        ...
        @load="onLoad(1)"
      >
        <r-cell v-for="item in list[1].items" :key="item" :title="item" />
      </r-list>
    </r-tab>

    <r-tab :title="t('pullRefresh')">
      <r-pull-refresh v-model="list[2].refreshing" @refresh="onRefresh(2)">
        <r-list
          ...
          @load="onLoad(2)"
        >
          <r-cell v-for="item in list[2].items" :key="item" :title="item" />
        </r-list>
      </r-pull-refresh>
    </r-tab>
  </r-tabs>
