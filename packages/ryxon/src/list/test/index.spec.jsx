...
test('should reload after clicking the error text', async () => {
  ...
  wrapper.find('.r-list__error-text').trigger('click');
...
test('should render finished text when finished prop is true', async () => {
  ...
  expect(wrapper.emitted('load')).toBeFalsy();
  expect(wrapper.emitted('update:loading')).toBeFalsy();
  expect(wrapper.find('.r-list__finished-text').exists()).toBeTruthy();

  await wrapper.setProps({ finished: false });
  await later();
  expect(wrapper.emitted('load')).toBeTruthy();
  expect(wrapper.emitted('update:loading')).toBeTruthy();
  expect(wrapper.find('.r-list__finished-text').exists()).toBeFalsy();
});
...
test('should render correctly when direction is up', async () => {
  ...
  let children = wrapper.findAll('.r-list > div');
  expect(children[0].classes()).toContain('r-list__placeholder');
  ...
  children = wrapper.findAll('.r-list > div');
  expect(children[0].classes()).toContain('list-item');
  expect(children[1].classes()).toContain('r-list__placeholder');
});
...
// https://github.com/PeterPanY/ryxon/issues/9017
