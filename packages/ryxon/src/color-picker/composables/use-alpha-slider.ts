...
export const useAlphaSliderDOM = (
  ...
  const [, bem, , isBem] = createNamespace('color-alpha-slider')
  // refs
  ...
  const rootKls = computed(() => [bem(), isBem('vertical', props.vertical)])
  const barKls = computed(() => bem('bar'))
  const thumbKls = computed(() => bem('thumb'))
