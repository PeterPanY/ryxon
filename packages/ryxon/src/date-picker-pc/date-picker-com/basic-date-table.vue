<template>
  <table
    role="grid"
    :aria-label="t('rDatepicker.dateTablePrompt')"
    cellspacing="0"
    cellpadding="0"
    :class="[bem(), { 'is-week-mode': selectionMode === 'week' }]"
  ...
  >
    <tbody ref="tbodyRef">
      <tr>
        <th v-if="showWeekNumber" scope="col">{{ t('rDatepicker.week') }}</th>
        <th
          v-for="(week, key) in WEEKS"
          :key="key"
          scope="col"
          :aria-label="t('rDatepicker.weeksFull.' + week)"
        >
          {{ t('rDatepicker.weeks.' + week) }}
        </th>
      </tr>
      <tr
        v-for="(row, rowKey) in rows"
        :key="rowKey"
        :class="[bem('row'), { current: isWeekActive(row[1]) }]"
      >
        <td
...
import { newCastArray, createNamespace } from '../../utils'
...
import RDatePickerCell from './basic-cell-render'
  ...
  setup(props, { emit }) {
    ...
    const selectedDate = computed(() =>
      props.selectionMode === 'dates'
        ? (newCastArray(props.parsedValue) as Dayjs[])
        : ([] as Dayjs[])
    )
    ...
    const handlePickDate = (
      ...
      if (props.selectionMode === 'range') {
        ...
        const newValue = cell.selected
          ? newCastArray(props.parsedValue).filter(
              (d) => d?.valueOf() !== newDate.valueOf()
            )
          : newCastArray(props.parsedValue).concat([newDate])
        emit('pick', newValue)
      }
    }
