<template>
  <div class="r-doc-header">
    <div class="r-doc-row">
      <div class="r-doc-header__top">
        <a class="r-doc-header__logo">
          <img :src="config.logo" />
          <span class="r-doc-header__title">{{ config.title }}</span>
          <span v-if="config.subtitle" class="r-doc-header__subtitle">
            {{ config.subtitle }}
          </span>
        </a>

        <ul class="r-doc-header__top-nav">
          <li
            v-for="(item, index) in config.links"
            :key="index"
            class="r-doc-header__top-nav-item"
          >
            <a class="r-doc-header__link" target="_blank" :href="item.url">
          ...
          >
            ...
            >
              {{ packageVersion }}
              <transition name="r-doc-dropdown">
                <div v-if="showVersionPop" class="r-doc-header__version-pop">
                  <div
                    v-for="(item, index) in versions"
                    :key="index"
                    class="r-doc-header__version-pop-item"
          ...
          <li v-if="langLabel && langLink" class="r-doc-header__top-nav-item">
            <a class="r-doc-header__cube" :href="langLink">
              {{ langLabel }}
            </a>
          </li>
        </ul>
...
.r-doc-header {
  ...
  &__top {
    display: flex;
    align-items: center;
    height: var(--r-doc-header-top-height);
    padding: 0 var(--r-doc-padding);
