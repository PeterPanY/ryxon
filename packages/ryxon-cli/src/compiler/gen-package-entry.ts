...
import { SRC_DIR, getPackageJson, getRyxonConfig } from '../common/constant.js';
...
export function genPackageEntry({
  ...
  const namedExport = ryxonConfig.build?.namedExport || false;
  const skipInstall = (ryxonConfig.build?.skipInstall || []).map(pascalize);
