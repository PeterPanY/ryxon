...
import { pascalize, getRyxonConfig, normalizePath } from '../common/index.js';
import {
  SRC_DIR,
  DOCS_DIR,
  getPackageJson,
...
function resolveDocuments(components: string[]): DocumentItem[] {
  ...
  const { locales, defaultLang } = ryxonConfig.site;
...
function genRyxonConfigContent() {
  const content = readFileSync(RYXON_CONFIG_FILE, 'utf-8');
