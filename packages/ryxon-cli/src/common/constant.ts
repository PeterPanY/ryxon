...
function findRootDir(dir: string): string {
  if (existsSync(join(dir, 'ryxon.config.mjs'))) {
    return dir;
  }
...
export const RYXON_CONFIG_FILE = join(ROOT, 'ryxon.config.mjs');
...
async function getRyxonConfigAsync() {
  try {
    // https://github.com/nodejs/node/issues/31710
    // absolute file paths don't work on Windows
    return (await import(pathToFileURL(RYXON_CONFIG_FILE).href)).default;
  } catch (err) {
    return {};
  }
}

const ryxonConfig = await getRyxonConfigAsync();

export function getRyxonConfig() {
  return ryxonConfig;
}

function getSrcDir() {
  const ryxonConfig = getRyxonConfig();
  const srcDir = ryxonConfig.build?.srcDir;
